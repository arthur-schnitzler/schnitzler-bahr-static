...
for x in tqdm(files, total=len(files)):
    ...
    cfts_record["resolver"] = (
        f"https://acdh-oeaw.github.io/bahr-static/{record['id']}.html"
    )
    record["rec_id"] = os.path.split(x)[-1]
    cfts_record["rec_id"] = record["rec_id"]
    record["title"] = " ".join(
        " ".join(doc.any_xpath(".//tei:titleStmt/tei:title[2]//text()")).split()
    )
    ...
    record["persons"] = [
        ...
        for x in doc.any_xpath(".//tei:back//tei:person/tei:persName[1]")
    ]
    cfts_record["persons"] = record["persons"]
    record["places"] = [
        " ".join(" ".join(x.xpath(".//text()")).split())
        for x in doc.any_xpath(".//tei:back//tei:place[@xml:id]/tei:placeName[1]")
    ]
    cfts_record["places"] = record["places"]
    record["orgs"] = [
        " ".join(" ".join(x.xpath(".//text()")).split())
        for x in doc.any_xpath(".//tei:back//tei:org[@xml:id]/tei:orgName[1]")
    ]
...
try:
    ...
    print(
        "Warning: Timeout while indexing CFTS collection. The index may be incomplete."
    )
except Exception as e:
